...
class MainWindow(QMainWindow):
    ...
    def _build_center_panel(self, center: QVBoxLayout) -> None:
        ...
        # Row 0: Force Channel (for current source)
        self.lbl_force = QLabel("Force Ch (Bank 1)")
        el.addWidget(self.lbl_force, 0, 0)
        ...
        self.spin_ch_pos.setToolTip("Channel for current source (Bank 1: 1001-1020). For Internal DMM, this is the only channel used.")
        el.addWidget(self.spin_ch_pos, 0, 1)

        # Row 0: Sense Channel (for voltage meter)
        self.lbl_sense = QLabel("Sense Ch (Bank 2)")
        el.addWidget(self.lbl_sense, 0, 2)
        ...
        self.spin_ch_neg.setToolTip("Channel for voltage meter (Bank 2: 1021-1040). Not used for Internal DMM.")
    ...
    def _on_measurement_mode_changed(self, index):
        ...
        if index == 0:  # Internal DMM
            ...
            self.lbl_voltage_set.hide()
            self.spin_voltage.hide()
            # Update labels for Internal DMM mode
            ...
            self.lbl_voltage_set.hide()
            self.spin_voltage.hide()
            # Update labels for external instrument mode
